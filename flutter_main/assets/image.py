...
def path_msg_write(path):
    ...
    for path_image in path_image_list:
        ...
        # 这里替换文件名中的 '-'
        s2 += pathgroup[lenght - 1].split('.')[0].replace('-', '_').capitalize()
        s2 = s2.replace(r'.', '')
...
def convert(one_string, space_character):
    # 先将字符串中的 '-' 替换为 '_'
    one_string = one_string.replace('-', '_')
...
def write_yaml(path):
    ...
    file = open(yaml_path, 'r', encoding='gbk')    # w 的含义为可进行读写
    line = file.readlines()
    file.close()
    ...
    yamlfile = open(yaml_path, 'w', encoding='gbk')    # w 的含义为可进行读写
