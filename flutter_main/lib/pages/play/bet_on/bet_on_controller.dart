...
import 'package:flutter_main/utils/my_queue.dart';
import 'package:flutter_main/utils/utils.dart';
import 'package:get/get.dart';
import 'package:flutter_main/models/ws/ws_401_model.dart' as ws_401_model;

class BetOnController extends GetxController {
  /// 获取ws控制器实例
  var wsGlobalController = Get.find<WebsocketController>();

  /// 入桌后更新的路子图model
  Rx<Ws116RoadGameTableDetailModel> ws116Model =
      Ws116RoadGameTableDetailModel().obs;

  /// 限红下限
  /// 项目缓存的最新桌台集合 map-BetPointLimit 数据
  /// key 为牌桌 id  betPointId
  RxMap<int, BetPointLimit> mTableBetPointLimitMapObs =
      RxMap<int, BetPointLimit>({});

  /// 牌靴限制
  /// 项目缓存的最新桌台集合 map-BootNumberLimit 数据
  /// key 为牌桌 id  betPointId
  RxMap<int, BootNumberLimit> mTableBootNumberLimitMapObs =
      RxMap<int, BootNumberLimit>({});

  /// 登录后返回的 10000model
  var currentWs10000Model = Ws10000Model();

  RxString tableNoObs = 'GC02411758F'.obs;
  RxString tableBootLimit = '20～5000'.obs;

  RxInt mRoundId = 0.obs;

  /// 复用按钮 文字  true：复用  false：X2
  RxBool isRepeatObs = true.obs;

  /// 记录当前闲 和 庄 总的下注金额
  int betOnBottomTotalAmount = 0;

  /// 根据 betPointId 更新 小玩法  人数和金额
  RxMap<int, TableBetPointDetail> tableBetPointDetailMapObs =
      RxMap<int, TableBetPointDetail>({});

  /// 返利数据
  /// key 为 betPointId
  RxMap<int, ReturnProfitBetPointDTOS> profitBetPointDTOSMapObs =
      RxMap<int, ReturnProfitBetPointDTOS>({});

  /// 其他人下注信息
  /// key 为 playerId
  Map<int, TableBetInfoModel> tableBetInfoModelMap = {};
  ...
  /// 保存最后一次 确认 下注的信息
  /// 上次投注信息  复用按钮的时候取用
  List<BetInfosBo> betInfosListLast = [];

  /// 点击重复保存下 每次下注记录下 下注信息
  List<BetInfosBo> betInfosListCurrent = [];
  ...
  late WsGameStatusModel? mWsGameStatusModel;

  /// 确认按钮显示 当前下注金额
  RxInt confirmBetAmount = 0.obs;
  ...
  void onTapCancel() async {
    ...
    if (104 != mWsGameStatusModel?.protocolId) {
  ...
  updateHandleGameStatusModel(WsGameStatusModel onData) {
    mWsGameStatusModel = onData;
    LogUtil.log(
        "--BetOnController--updateHandleGameStatusModel-onData.gameStatus=${onData.protocolId}");
    if (107 == mWsGameStatusModel?.protocolId) {
      //监听服务器主动推送107（结算 ）-结算中
      /// 发送完 投注记录  清空
  ...
  void initRoadData() {
    ...
    ws116Model.value.roadPaper?.bigRoad = roadPaper10053?.bigRoad;
    ws116Model.value.roadPaper?.winPointPlateRoad =
        roadPaper10053?.winPointPlateRoad;
    ws116Model.value.roadPaper?.dragonBonusPlateRoad =
        roadPaper10053?.dragonBonusPlateRoad;
    ws116Model.value.roadPaper?.bigPairRoad = roadPaper10053?.bigPairRoad;
    ws116Model.value.roadPaper?.beatPlateRoad = roadPaper10053?.beatPlateRoad;
    ws116Model.value.roadPaper?.winLoseRoad = roadPaper10053?.winLoseRoad;
    ws116Model.value.roadPaper?.cockroachPig = roadPaper10053?.cockroachPig;
    ws116Model.value.roadPaper?.smallRoad = roadPaper10053?.smallRoad;
    ws116Model.value.roadPaper?.bigEyeBoy = roadPaper10053?.bigEyeBoy;
    ws116Model.value.roadPaper?.bigSmallPlateRoad =
        roadPaper10053?.bigSmallPlateRoad;
    ws116Model.value.roadPaper?.newBigPairRoad = roadPaper10053?.newBigPairRoad;
    update();
  }
  ...
  updateTableNoObs(int roundId, String roundNo) {
    ...
    if (mRoundId.value != roundId) {
      mRoundId.value = roundId;
    }
  }
  ...
  bool isBetOn(int betPointId, int betAmount) {
    ...
    if (WebsocketController.controller.mBalance.value < min) {
  ...
  void updateWs271Model(Ws271Model value) {
    ...
    if (cancelBettings != null && cancelBettings.isNotEmpty) {
      for (var value1 in cancelBettings) {
        ...
        //todo 法码取消动画
        ChipFlyController.controller.cancelPlayChip(value1.cancelAmount!, value1.betPointId);
      }
    }
  }
